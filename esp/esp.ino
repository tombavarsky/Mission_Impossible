...
void loop() {
  ...
  GETRequest(client, gameResult, true);
}
...
void ConnectWebsite (WiFiClientSecure client, bool toSerial) {
  ...
  if (!client.connect(host, httpPort)) {
    if (toSerial) Serial.println("connection failed");
    return;
  }
  if (toSerial) Serial.println("connection successful");
}
...
/*
   ...
   toSerial - true if serial monitor status updates are desired
*/
String GETRequest (WiFiClientSecure client, int result, bool toSerial) {
  ...
  client.print(String("GET ") + url + "stand=" + STAND_NAME + "&time=" + time + "&touchCount=" + touch_count + " HTTP/1.1\r\n" +
               "Host: " + host + "\r\n" +
               "Connection: close\r\n\r\n");
  if (toSerial) {
    Serial.println();
  ...
  while (client.connected()) {
    ...
    if (toSerial) Serial.println("reading");
    if (line == "\r") break;
  }
  ...
  if (toSerial) Serial.println(text);
  return text;
}
